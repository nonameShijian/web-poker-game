...
public class RedisUtils {
    ...
    public void del(String... key) {
        if (key != null && key.length > 0) {
            if (key.length == 1) {
                redisStringTemplate.delete(key[0]);
            } else {
                redisStringTemplate.delete((Collection<String>) CollectionUtils.arrayToList(key));
            }
        }
    }
    ...
    public boolean set(String key, Object value, long time) {
        try {
            if (time > 0) {
                redisStringTemplate.opsForValue().set(key, value, time,
                        TimeUnit.SECONDS);
            } else {
                set(key, value);
            }
    ...
    public Object hget(String key, String item) {
        return redisStringTemplate.opsForHash().get(key, item);
    }
    /**
     * 获取hashKey对应的所有键值
     * @param key 键
...
public void hdel(String key, Object... item) {
    redisStringTemplate.opsForHash().delete(key, item);
}
    /**
     * 判断hash表中是否有该项的值
     *
    ...
    public boolean hHasKey(String key, String item) {
        return redisStringTemplate.opsForHash().hasKey(key, item);
    }
    /**
    ...
    public double hincr(String key, String item, double by) {
        return redisStringTemplate.opsForHash().increment(key, item, by);
    }
    /**
     * hash递减
     *
...
// ============================set=============================
    ...
    public long sSet(String key, Object... values) {
        try {
            return redisStringTemplate.opsForSet().add(key, values);
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
        }
    ...
    public long sSetAndTime(String key, long time, Object... values) {
        try {
            Long count = redisStringTemplate.opsForSet().add(key, values);
            if (time > 0)
                expire(key, time);
            return count;
        } catch (Exception e) {
    ...
    public long setRemove(String key, Object... values) {
        try {
            Long count = redisStringTemplate.opsForSet().remove(key, values);
            return count;
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
    ...
    public boolean lSet(String key, List<Object> value) {
        try {
            redisStringTemplate.opsForList().rightPushAll(key, value);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    public boolean lUpdateIndex(String key, long index, Object value) {
        try {
            redisStringTemplate.opsForList().set(key, index, value);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    public long lRemove(String key, long count, Object value) {
        try {
            Long remove = redisStringTemplate.opsForList().remove(key, count,
                    value);
            return remove;
        } catch (Exception e) {
            e.printStackTrace();
